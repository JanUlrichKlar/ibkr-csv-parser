% Main function to process Interactive Brokers CSV reports
% Returns two structures: infoTables (account info) and finTable (financial data)
function [infoTables, finTable] = processIBKRCSV(filename)
    % Import and preprocess the CSV file (handle quotes, encoding, and format)
    T = importAndConvertIBCSV(filename);

    % Find index for each subtable
    % Locate section headers in column 2 (to split CSV into subtables)
    headerIndices = find(strcmp(strtrim(T{:,2}), "Header"));
    headerIndices(:,2) = [headerIndices(2:end)-1; size(T, 1)];
    
    % Language detection based on known header keywords
    ...
    % Find rows containing field name/value labels
    ...
    % Extract metadata/info sections into tables (e.g., Account Info)
    ...
    % Extract tabular financial data (e.g., Trades, Positions)
...
% === Helper: importAndConvertIBCSV ===
% Custom CSV reader that handles quotes, encoding issues, and inconsistent delimiters
...
% === Helper: extractFromCellTable ===
% Converts a table with mixed types (strings, numbers, datetimes) into clean MATLAB table
...
% === Helper: ifelse ===
% A simple inline conditional used for cleaning empty entries
